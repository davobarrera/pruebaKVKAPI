...
export default function ({DI, router}){
    const service = new TrackService({DI})
    router.get('/all/view', findAll(service));
    router.get('/procedure/tracks/without/artist', callTracksWithoutArtist(service));
...
function findAll(service) {
    return async (req, res) => {
        res.json(await service.getAllTracksView());
    }
}

function callTracksWithoutArtist(service) {
    return async (req, res) => {
        res.json(await service.callTracksWithoutArtist());
    }
}
